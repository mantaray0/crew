...
4. **Write the plan.** For each phase, write a **numbered** plan file into the numbered milestone folder `.planning/plans/<n>_<milestone-slug>/<id>-<kebab-title>.md` (folder prefixed with the milestone number `<n>`; file named by the roadmap phase id, e.g. `1_fundament/1.2-db-schema.md`) with a **Spec** head (carried from the `_spec.md` brief or the source ticket) followed by a **Plan** body: affected files, tasks (Action / pattern to mirror / validation command), risks, and this phase's verify configuration. For a **new project**, create one `<n>_<milestone-slug>/` folder per roadmap milestone; for a **feature**, write into the brief's existing `<n>_<slug>/` folder. Follow `clarify.specArtifact`. Once a brief's phases are all captured, the `_spec.md` may be removed (it has become the numbered plans).
