---
description: Run the verify pipeline on the current change — verify → review → harden → simplify — in fresh sub-agent contexts, per config.
...
2. **Resolve steps.** Read `config.verify.default` (and any phase override). Default: `["verify","review","harden","simplify"]`.
3. **Run each step in a fresh sub-agent context**, choosing the model per `config.models` (see `model-management`):
   - **verify** — run the project's tests / build / typecheck (from `PROJECT.md`); test-strictness per `config.testing.policy`.
