...
Explicitly run the verification pipeline (it also runs automatically inside `/crew:execute` per `config.workflow.execute.verify`). Uses the `verification-loop` skill.
