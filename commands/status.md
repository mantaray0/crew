...
5. Read the backlog item files `.planning/backlog/*.md` (frontmatter — see `crew-context` → *The backlog*; there is no `BACKLOG.md`) and show a compact backlog section: item count plus the first ~5 titles **by priority** (`high` first). On overflow, append `… (+k more — run /crew:backlog list)`. If the folder is empty or absent, note it. Read-only — never write to the backlog.
