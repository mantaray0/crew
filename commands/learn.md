---
description: Learn — distill reusable patterns and decisions from finished work into proposed skills or tags for the global registry.
...
**Catch-up awareness.** When invoked directly, check the milestone's step state (`LOG.md`) and **offer any missing earlier close-out step** (e.g. `ship`) per its `run` — per the canonical **Catch-up rule** in `crew-conventions` (never forced, no dependency graph).
