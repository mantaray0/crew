...
2. **Project reconcile** (unless `$ARGUMENTS` is `global`): if `.planning/config.json` exists, run the reconcile procedure from `crew-config` → **Config versioning & migration** against it — apply the **Known migrations** first (so renamed/split keys keep their value — including the version-gated **`testing.policy` → top-level `testingPolicy`** rename, value 1:1), then schema-diff (classify keys new / removed / unchanged), ask per new field (**inherit-first** — the inherit choice first and pre-selected, writing the sentinel `"inherit"` when chosen), offer to drop removed keys, and stamp `crewVersion` to the plugin version. Per *Inherit-first writes* the reconcile also runs the **one-time inherit-first cleanup** of legacy over-seeded fields at the inherit-first transition (gated on `crewVersion` < `0.16.0`), offers the **opt-in revisit pass** (re-walk existing parameters, current value pre-selected), and can **reset** any single override back to inherit (set to `"inherit"`) — all delegated to `crew-config`, not restated here. **Also offer once — opt-in — to expand a minimal pre-sentinel config to the full inherit form** (`crew-config` → *Known migrations*, the explicit-inherit-sentinel expansion entry, version-gated): every currently-absent inheritable leaf written as `"inherit"`, existing concrete freezes untouched — never forced; a declined config stays minimal and equally correct (missing ≡ `"inherit"`). **Also offer the plans-layout migration** — same as `/crew:init` step 1 (see `planning` for the file-pattern details): a pure `mv`, no content change. **Also offer the backlog-folder content migration** (`crew-config` → *Known migrations*, the `BACKLOG.md` → `backlog/*.md` entry, version-gated, idempotent, interactive on ambiguity): convert a legacy free-text `BACKLOG.md` into `backlog/*.md` item files and remove the old file after confirmation — delegated to `crew-config`, not restated here.
...
4. **Report** the diff and what changed compactly — per level: version before/after, new fields resolved, removed fields dropped, migrations applied. For each **inheriting** field, show its **resolved value + source** in the one canonical form (`crew-config` → *Surfacing the source*: `` `language.files`: inherit → `de` (from global) `` · `` `responseStyle`: inherit → `concise` (from default) ``) so the user sees what each `"inherit"` resolves to and from where.
