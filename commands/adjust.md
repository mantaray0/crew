...
4. **Update dependencies.** If the change affects inter-phase dependencies (for parallel dispatch), update them in the affected `plans/<milestone-slug>/<id>-<title>.md` and the roadmap notes.
5. **Backlog triage.** If pulling from `BACKLOG.md`, move the item into the roadmap (and create/extend its `plans/<milestone-slug>/` plan Spec head if it needs planning) and remove it from the backlog.
