...
6. **Commit or ignore `.planning/`.** Single-select: **commit** `.planning/` (recommended — shareable, part of project history, readable by PM integrations) or **gitignore** it (local-only)? If **gitignore**, add a `.planning/` line to the project's `.gitignore` (create the file if missing). If **commit**, make sure `.planning/` is not ignored.
7. **Scaffold `.planning/`:**
...
8. **Confirm** and point to `/crew:brief` (new feature) or `/crew:plan`.
