...
Whenever the loop **continues** to a next phase (the default `loop: all`, or `auto`), it runs each phase in a **fresh context** — `/clear` between phases, then re-enter. This is the documented, **reliable** behavior, not an `auto`-only side effect: continuity rides entirely on `.planning/` state (`ROADMAP.md` + `claims.json` + the phase plan), so the re-invocation re-derives the exact next phase and clearing loses nothing. It matters most under active **isolation** — the milestone/phase worktree and claim are re-read from state, not held in conversation memory — so a long milestone never degrades into one bloated context. The self-continuation mechanic — for both the bare-default `loop: all` and the `auto` argument — is described under *auto* below; a **single-phase** run (`loop: one` or a named phase id) doesn't self-continue — it hands the `/clear` to the user (see *Hand-off*).
...
3. **Otherwise stop — or chain.** No pending phases left, or a stop condition fired → handle the milestone end per **`workflow.mode`** (*Milestone end* above): the **user-test gate** fires first (per its cadence) and must pass, then under `manual` (default) report and **suggest** `/crew:finish`; under `auto` advance the chain (Ship → Learn → Complete per each step's `run`). On a stop condition (deviation/critical finding, or a failed user-test checkpoint), do **not** clear, re-invoke, or chain.
