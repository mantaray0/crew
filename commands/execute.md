...
2. **Milestone-boundary guard, then claim.** If the next `[ ]` phase belongs to a *different* milestone than the one just finished **and** that previous milestone is fully `[x]` (treat `[~]` deferred phases as not-blocking), **pause before claiming** and **offer `/crew:complete-milestone`** (audit → summary → archive) first — the user may skip and proceed. This gives the one-phase default the same milestone awareness `auto`/`dispatch` already have; it only *offers*, it never self-completes (`config.git.autoPush`/`autoPR` stay false). Then **claim:** mark the phase `[>]` in `ROADMAP.md` and record the claim in `.planning/claims.json` (so parallel instances don't collide).
