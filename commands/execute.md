...
2. **Milestone-boundary guard, then claim.** If the next `[ ]` phase belongs to a *different* milestone than the one just finished **and** that previous milestone is fully `[x]` (treat `[~]` deferred phases as not-blocking), **pause before claiming** and **offer `/crew:finish`** (the config-gated Ship → Retro → Complete close-out strand) first — the user may skip and proceed. This gives the one-phase default the same milestone awareness `auto`/`dispatch` already have; it only *offers*, it never self-finishes (`config.git.autoPush`/`autoPR` stay false). Then **claim:** mark the phase `[>]` in `ROADMAP.md` and record the claim in `.planning/claims.json` (so parallel instances don't collide).
3. **Implement.** Do exactly what the plan's phase specifies, mirroring existing patterns. Model = `config.models.execution` (or auto). **Deviation handling (`execute.onDeviation`):** decide small in-intent deviations yourself and note them in the log; on a real problem, ambiguity, or scope change, **stop and ask**.
...
> **Done?** Run `/clear` to reset the context, then `/crew:execute` again for the next phase — or `/crew:finish` once the milestone is done (the config-gated Ship → Retro → Complete close-out strand).
...
- **Never self-ship, self-complete, or self-finish.** `config.git.autoPush`/`autoPR` stay false; do **not** run `/crew:ship`, `/crew:complete`, or `/crew:finish` yourself.
- **At the end, report and suggest** the next step — **`/crew:finish`** (the config-gated Ship → Retro → Complete close-out strand) — leave the deliberate action to the user.
...
3. **Dispatch a wave.** For each independent phase in the current wave, up to `config.execute.maxConcurrent` at a time:
