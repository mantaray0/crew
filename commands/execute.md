...
1. **Load context.** Read `.planning/PROJECT.md`, `.planning/ROADMAP.md` (find the active `[>]` phase, else the next `[ ]`), its plan file under `.planning/plans/<n>_<milestone-slug>/<id>-…md` (glob `plans/**/*.md` to locate it by phase id), and the last entries of `.planning/LOG.md`. From these the **exact next step** must be unambiguous. If it is not, ask.
