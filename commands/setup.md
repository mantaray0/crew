...
3. **Global `config.json`.** Decide whether to write one at all (single-select). If yes, go through the config groups (`models`, `git`, `notifications`, `tasks.provider`, `execution`, …) **one at a time** — for each, single-select "keep default vs override", and on override ask the specific value (select / free-text). Write only the values the user confirmed; the rest stay as built-in defaults.
