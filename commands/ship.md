---
description: Carry a verified change to a release — version, commit, tag, push, PR, and (when enabled) deploy — driven by config.ship and bounded by config.git.
...
1. **Read config.** Read `config.ship` (enabled/provider/tagPattern/environments/runDeploy/releaseTool/finishRelease), `config.git`, and `reference/deploy.md` (if present). If `config.ship.enabled` is `false`, explain how to enable it (`/crew:init` → deploy, or set `config.ship.enabled`) and **stop**. Resolve `releaseTool` — if `auto`, detect it from the repo per `crew-deploy` → Release mechanics.
...
3. **Release per `releaseTool`** (see `crew-deploy` → Release mechanics; every git step defers to `config.git`):
   - **`manual`** — version locally (`npm version` / language equivalent, command from `reference/deploy.md`) → release commit (`config.git.commitStyle`; if `autoCommitPerPhase` is false, **ask**) → tag from `config.ship.tagPattern` (e.g. `v1.4.0`).
...
6. **Finish release (bot-PR tools only, `config.ship.finishRelease ≠ off`).** For `changesets`/`release-please`: after the push the CI bot opens a version/release-PR. If one is open, merge it per `finishRelease` (`ask` → confirm first, `auto` → proceed) → CI tags + releases. Never on a red verify.
7. **Deploy (only if `config.ship.runDeploy ≠ off`).** Run the deploy command from `reference/deploy.md` for the target environment (the optional `$ARGUMENTS`, else the default). With `ask`, confirm first; with `auto`, proceed; never run it on a red verify.
