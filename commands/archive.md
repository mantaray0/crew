...
3. **Confirm.** Show what will move (the roadmap section + `plans/<slug>/`) and where, then proceed.
4. **Move state** (a pure `mv` + text edit, no content change):
   - Append the milestone's `ROADMAP.md` section to `.planning/archive/roadmap-<slug>.md` (create the file/dir if missing).
   - Move `.planning/plans/<slug>/` → `.planning/archive/plans/<slug>/`.
   - Replace the milestone's section in `ROADMAP.md` with a one-liner: `## Meilenstein N: <Titel> — ✓ archiviert YYYY-MM-DD → archive/roadmap-<slug>.md`.
