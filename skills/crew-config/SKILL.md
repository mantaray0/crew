...
```jsonc
{
  ...
  "notifications": {
    ...
    "channel": "off"                   // "off" | "push:ntfy" | "push:pushover" — push only; the host handles local/desktop notifications
  },
...
**`notifications.channel` enum reduced — local `os` channel removed** (crew notifies **push-only**; local/desktop notification is the host's job) — gated on `crewVersion` predating the push-only-notifications release (`< 0.22.0`; if ship cuts a different version, align this number there — describe it as "the push-only-notifications release", never by an internal label). The `os` channel shelled out to `osascript`/`notify-send`; on macOS the OS attributes such a notification to the invoking script host, so it arrived under a foreign icon and clicking it opened the Script Editor instead of the session — an unfixable attribution problem for a plugin (a proper sender needs a signed app bundle). Claude Code already ships its own local notifications, so nothing is lost. Map losslessly, **never silently drop**: a frozen `"os"` → `"off"` (the axis keeps `off` · `push:ntfy` · `push:pushover`). **Also flag the default flip:** the built-in default moves `"os"` → `"off"`, so a config that *inherits* `channel` (absent or `"inherit"`) needs no rewrite but does change behavior — it stops notifying locally. Surface both in the reconcile summary, and — when the config inherits or was mapped to `off` — offer `push:ntfy` (needs `CREW_NTFY_TOPIC`) as the remaining way to get notified off-machine.
