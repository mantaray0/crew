...
```jsonc
{
  ...
  "git": {
    ...
    "isolation": "off",                // | "worktree-per-milestone" | "branch-per-milestone" | "worktree-per-phase" | "branch-per-phase" — mechanism prefix (worktree-/branch-) × granularity suffix; off = linear (opt-in; worktree is the preferred mechanism)
