...
4. **Challenge at the configured intensity** (`config.brief.intensity`):
...
5. **Respect breadth** (`config.brief.depth`, orthogonal to intensity): `light` = only the load-bearing questions; `normal` = the decision tree's main branches; `deep` = edge cases and failure modes too.
