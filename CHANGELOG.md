...
- [#41](https://github.com/mantaray0/crew/pull/41) [`8cf658f`](https://github.com/mantaray0/crew/commit/8cf658f1671f7ffe5fb528ef6663986b3c54c9df) Thanks [@mantaray0](https://github.com/mantaray0)! - Fix `workflow.brief.depth`/`intensity` having no effect, and `/crew:brief` drifting away from the `roast-me` skill.
  ...
  - `/crew:plan` states its scope line explicitly: it plans the _how_ and deliberately runs no Roast-Me. Structural ideas the brief parks now survive the `/clear` between the two commands via an optional **`Notes for planning`** section in `_spec.md` — input for planning, never a requirement.
